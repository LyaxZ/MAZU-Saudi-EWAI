"""生成所有教师 Ground Truth 事件的可视化 HTML（下拉切换）"""
...
# 教师 Ground Truth 事件
EVENTS = [
    ("2025-05-04", "dust_wind",   "哈布尘暴 (Haboob)",    24.5, 28.5, 41.0, 45.0),
    ("2025-05-16", "dust_wind",   "全国持续性沙尘",     16.0, 32.0, 34.0, 56.0),
    ("2025-05-25", "extreme_heat","52.2°C 破纪录高温",   16.0, 32.0, 34.0, 56.0),
    ("2025-06-01", "extreme_heat","朝觐季 47°C 高温",    16.0, 32.0, 34.0, 56.0),
    ("2025-06-30", "dust_wind",   "东部/汉志持续性沙尘", 16.0, 32.0, 34.0, 56.0),
    ("2025-08-14", "flash_flood", "塔伊夫冰雹洪水",      20.5, 22.0, 40.0, 42.0),
    ("2025-08-27", "flash_flood", "阿西尔/吉赞/纳季兰山洪", 16.5, 19.5, 42.0, 45.0),
    ("2025-12-09", "flash_flood", "吉达历史性洪水 (179mm/6h)", 21.0, 22.5, 38.5, 40.0),
]
...
HTML = f"""<!DOCTYPE html><html lang="zh-CN"><head><meta charset="utf-8"><title>MAZU KG — 教师 Ground Truth 事件可视化</title>
<style>
...
<div class="top">
<h1>🛰️ MAZU KG — 真实灾害事件可视化</h1>
<select id="sel" onchange="sw(this.value)">
{''.join(opts)}
</select>
<div class="info">{len(EVENTS)} 个事件 · 教师 Ground Truth · 点击下拉切换</div>
</div>
{''.join(parts)}
<script>
...
print(f"✅ {OUT} ({len(HTML):,} chars) | {len(EVENTS)} 个事件")
